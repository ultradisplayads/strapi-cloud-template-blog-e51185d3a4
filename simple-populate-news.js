...
async function populateNewsSources() {
  ...
  for (const source of pattayaNewsSources) {
    try {
      ...
      const existingResponse = await axios.get(`${API_BASE_URL}/api/news-sources?filters[name][$eq]=${encodeURIComponent(source.name)}`);
      const existingSources = existingResponse.data.data || [];

      if (existingSources.length > 0) {
        // Update existing source
        const existing = existingSources[0];
        const updateResponse = await axios.put(`${API_BASE_URL}/api/news-sources/${existing.id}`, {
          data: source
        });
        ...
        // Create new source
        const createResponse = await axios.post(`${API_BASE_URL}/api/news-sources`, {
          data: source
        });
      ...
      console.log(`❌ Failed to process ${source.name}:`);
