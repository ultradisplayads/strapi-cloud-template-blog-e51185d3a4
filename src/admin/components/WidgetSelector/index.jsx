...
const WidgetSelector = ({ name, value, onChange, attribute }) => {
  ...
  const handleCategoryToggle = (category) => {
    const categoryWidgets = widgetsByCategory[category].map(w => w.id);
    const allCategorySelected = categoryWidgets.every(id => selectedWidgets.includes(id));
    
    if (allCategorySelected) {
      // Remove all widgets from this category
      const newSelection = selectedWidgets.filter(id => !categoryWidgets.includes(id));
      setSelectedWidgets(newSelection);
      onChange({ target: { name, value: newSelection } });
    } else {
      // Add all widgets from this category
      const newSelection = [...new Set([...selectedWidgets, ...categoryWidgets])];
      setSelectedWidgets(newSelection);
      onChange({ target: { name, value: newSelection } });
    }
  };
    ...
    <div style={{ padding: '16px' }}>
      ...
      <div>
        {Object.entries(widgetsByCategory).map(([category, widgets]) => {
          ...
          const selectedInCategory = categoryWidgetIds.filter(id => selectedWidgets.includes(id));
          const isAllSelected = selectedInCategory.length === categoryWidgetIds.length;
          const isPartiallySelected = selectedInCategory.length > 0 && selectedInCategory.length < categoryWidgetIds.length;
              ...
              <div style={{ marginBottom: '12px' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 'bold' }}>
                  ...
                  <span style={{ fontSize: '14px' }}>
                    {category} ({selectedInCategory.length}/{categoryWidgetIds.length})
                  </span>
                </label>
              </div>
